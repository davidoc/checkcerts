...
for my $certfile (@certlist) {
    ...
    for my $entry (@$entries){
        if( is_member($entry->type(), ("DC","emailAddress") )) {    
            ...
            ok(($entry->is_printableString() or $entry->is_utf8string()), $entry->type() . ' SHOULD be printableString or utf8string (3.2.3) (actually: ' . $entry->encoding . ')');
        }
    }

    # 3.2.3 For certificates issued to networked entities, typically the (primary) FQDN of the server
    # is included in the commonName. 
    for my $entry (@$entries){
