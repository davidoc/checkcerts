...
for my $certfile (@certlist) {
    ...
    like($x509->sig_alg_name, '/sha-?1/i', 'Message digest SHOULD be SHA-1 (3.1) (It is actually:' . $x509->sig_alg_name .')');
    ...
    for my $entry (@$entries){
        if( is_member($entry->type(), ("DC","emailAddress") )) {    
            ok($entry->is_ia5string(), $entry->type() . ' SHOULD be ia5String (3.2.3) (It is actually: ' . $entry->encoding . ')');
        } else {
            ok(($entry->is_printableString() or $entry->is_utf8string()), $entry->type() . ' SHOULD be printableString or utf8string (3.2.3) (It is actually: ' . $entry->encoding . ')');
        }
    }
