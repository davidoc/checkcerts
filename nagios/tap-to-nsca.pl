...
tap-to-nsca - convert perl-style TAP test output to JUnit-style XML

=head1 SYNOPSIS

tap-to-nsca [--help|--man]
                [--[no]hidesummary]
...
=head1 EXAMPLE
    ...
    tap-to-nsca "make test" testxml/tests < tests.log
...
http://github.com/jmason/tap-to-nsca/tree/master
...
my $nscagen          = XML::Generator->new( ':pretty');
my $nscagenunescaped = XML::Generator->new( escape      => 'unescaped',
                                          conformance => 'strict',
                                          pretty      => 2
                                        );
my @properties = _get_properties($nscagen);
if ( defined $opts{'puretap'} ) {
   #
   # Instead of trying to parse everything in one pass, which fails if the
   # testplan is last, or if no plan is given, parse through the results for
   # the test cases and then construct the <testsuite> information from the TAP
   # and wrap it around the test cases. Ignore 'unknown' information.  [JL]
   #

   my @testcases = _parse_testcases( $tap, $nscagen );
   errorOut( $tap, $nscagen ) if $tap->parse_errors;
   #print $outfh $nscagen->testsuites(
   #                $nscagen->testsuite( { name     => $safe_suite_name,
   ...
   #                                      id       => 1 },
   #                                      @testcases ));
   print $outfh @testcases;

}
else {
   my $test_results    = _parse_tests( $tap, $nscagen );
   if ($opt_multifile) {
  _gen_junit_multifile_xml( $nscagen, \@properties, $test_results );
   } else {
       print $outfh _get_junit_xml( $nscagen, \@properties, $test_results );
   }
}
exit;
...
sub _get_junit_xml {
  my ( $nscagen, $properties, $test_results ) = @_;
...
sub errorOut {
   my $parser = shift;
   my $nscagen = shift;
   die "errorOut() needs some args"  unless $parser and $nscagen;
   my ($xml, @errors, $name);
   my $count = 1;
   foreach my $error ( $parser->parse_errors ) {
       $name = sprintf "%s%02d", 'Error_', $count++;
       $xml  = $nscagen->testcase( { name      => $name,
                                    classname => 'TestsNotRun.ParseError',
                                    time      => 0 },

                   $nscagen->error( { type    => 'TAPParseError',
                                     message => $error } ));
       push @errors, $xml;
   }
   print $outfh $nscagen->testsuites(
                   $nscagen->testsuite( { name     => 'TestsNotRun.ParseError',
                                         tests    => $tap->tests_planned,
                                         failures => 0,
...
#
# Construct an array of NSCA'd test cases
#
sub _parse_testcases {
   my $parser = shift;
   my $nscagen = shift;
   return () unless $parser and $nscagen;
   my ($name, $directive, $description, $nsca, @testcases);

   while ( my $result = $parser->next ) {
       my $t = $result->type;
       my $s = $result->as_string; $s =~ s/\s+$//;
       warn "DEBUG $t $s";

       if ( $result->is_bailout ) {
           $nsca  = $nscagen->testcase( { name      => 'BailOut',
                                        classname => "$safe_suite_name.Tests",
                                        time      => 0 },

                       $nscagen->error( { type    => 'BailOut',
       ...
       my $target="Grid-Ireland"; #TODO fetch this from the test run
       if ( $result->is_ok ) {
            $nsca = "$target\t$name\t0\t$description\n";
           #$nsca = $nscagen->testcase( { name      => $name,
           #                            classname => "$safe_suite_name.Tests",
           #                            time      => 0 } );
           push @testcases, $nsca;
       }
       else {
            $nsca = "$target\t$name\t2\t$description\n";
#           $nsca = $nscagen->testcase( { name      => $name,
#                                       classname => "$safe_suite_name.Tests",
#                                       time      => 0 },
#                      $nscagen->failure( { type    => 'TAPTestFailed',
  ...
  while ( my $r = $parser->next ) {
    ...
    if ($t eq 'unknown') {
      ...
      if ($s =~ /^Test Summary Report$/) {
        ...
        my $failure = $nscagen->failure({
          type => "OverallTestsFailed",
          message => $s
        }, "__FAILUREMESSAGETODO__");

        if (!$HIDE_TEST_SUMMARY_REPORT) {
          push @{$ctx->{test_cases}}, $nscagen->testcase($test_case, $failure);
        }
      }
      elsif ($s =~ /^(\S+?)\.\.\.+1\.\.(\d+?)\s*$/) {
      ...
      if ($s =~ /^not /i) {
        ...
        push @{$ctx->{test_cases}}, $nscagen->testcase($test_case);
      }
    }
...
sub _finish_test_block {
  ...
  push @{$ctx->{testsuites}}, $nscagenunescaped->testsuite($testsuite,
          @fixedcases,
          \("<system-out>\n".cdata($ctx->{sysout})."\n</system-out>"),
          \("<system-err />"));
...
sub cdata {
  my $s = shift;
  $s =~ s/\]\]>/\](warning: defanged by tap-to-nsca)\]>/gs;
  return '<![CDATA['.$s.']]>';
}
...
sub _get_properties {
    ...
    foreach my $key ( sort keys %ENV ) {
        push @props, $nscagen->property( { name => "$key", value => $ENV{$key} } );
    }
    return @props;
}
